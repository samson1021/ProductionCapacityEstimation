...
namespace mechanical.Services.ProductionCaseAssignmentServices
{
    public class ProductionCaseAssignmentServices : IProductionCaseAssignmentServices
    {
        ...
        public ProductionCaseAssignmentServices(CbeContext cbeContext, IMapper mapper, IHttpContextAccessor httpContextAccessor, IProductionCaseTimeLineService productionCaseTimeLineService)
        {
        ...
        public async Task<List<ProductionCaseAssignmentDto>> AssignProductionCheckerTeamleader(Guid userId, string selectedProductionIds, string employeeId)
        {
            ...
            ProductionCaseTimeLinePostDto caseTimeLinePostDto = null;
            foreach (Guid collateralId in collateralIdList)
            {
                ...
                if (collateral != null)
                {
                    ...
                    {
                        caseTimeLinePostDto = new ProductionCaseTimeLinePostDto()
                        {
                            ProductionCaseId = collateral.ProductionCaseId,
                    ...
                    {
                        collateralCaseId = collateral.ProductionCaseId;
                    }
                }
            ...
            if (caseTimeLinePostDto != null) await _productionCaseTimeLineService.CreateProductionCaseTimeLine(caseTimeLinePostDto);
        ...
        public async Task<List<ProductionCaseAssignmentDto>> SendProductionForValuation(string selectedProductionIds, string CenterId)
        {
            ...
            var CivilUser = await _cbeContext.CreateUsers.Include(res => res.District).FirstOrDefaultAsync(res => res.DistrictId == centerId && res.Department == "Aerospace" && (res.Role.Name == "Maker Manager" || res.Role.Name == "District Valuation Manager"));
            var MechanicalUser = await _cbeContext.CreateUsers.Include(res => res.District).FirstOrDefaultAsync(res => res.DistrictId == centerId && res.Department == "Marine" && (res.Role.Name == "Maker Manager" || res.Role.Name == "District Valuation Manager"));
            var AgricultureUser = await _cbeContext.CreateUsers.Include(res => res.District).FirstOrDefaultAsync(res => res.DistrictId == centerId && res.Department == "Railway" && (res.Role.Name == "Maker Manager" || res.Role.Name == "District Valuation Manager"));
