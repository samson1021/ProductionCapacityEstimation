    ...
    public class CollateralEstimationFeeService : ICollateralEstimationFeeService
    {
        ...
        public async Task<CollateralEstimationFeeDto> CreateCollateralEstimationFee(Guid UserId, CollateralEstimationFeeDto dto)
        {
            try
            {
                ...
                ////// 
                entity.PCEId = Guid.Parse("E1BBBE4A-F804-439A-A8E6-539232CCC6F0");
        ...
        public async Task<IEnumerable<CollateralEstimationFeeDto>> GetAllCollateralEstimationFees(Guid UserId, Guid PCEId)
        {
            try
            {
                var entities = await _cbeContext.CollateralEstimationFees.Where(f => f.PCEId == PCEId).ToListAsync();
        ...
        public async Task<bool> ValidateFees(Guid UserId, Guid PCEId)
        {
            try
            {
                var fees = await _cbeContext.CollateralEstimationFees
                    .Where(f => f.PCEId == PCEId && f.Status == FeeStatus.Pending)
                    .ToListAsync();

                if (!fees.Any())
                {
                    _logger.LogWarning("No pending fees found for case id {PCEId}", PCEId);
                    return false;
                }
        ...
        public async Task<bool> CommitFees(Guid UserId, Guid PCEId)
        {
            try
            {
                var fees = await _cbeContext.CollateralEstimationFees
                    .Where(f => f.PCEId == PCEId && f.Status == FeeStatus.Validated)
                    .ToListAsync();

                if (!fees.Any())
                {
                    _logger.LogWarning("No validated fees found for case id {PCEId}", PCEId);
                    return false;
                }
