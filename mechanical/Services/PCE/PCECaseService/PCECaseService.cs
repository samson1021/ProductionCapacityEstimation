       ...
       //abdu stsrt
        public async Task<CreateNewCaseCountDto> GetDashboardPCECaseCount()
        {
            ...
            return new CreateNewCaseCountDto()
            {
                ...
                PCSCompletedCaseCount = await _cbeContext.PCECases.Where(res => res.RMUserId == Guid.Parse(httpContext.Session.GetString("userId")) && res.ProductionCapacities.Any(collateral => collateral.CurrentStage == "Checker Officer" && collateral.CurrentStatus == "Complete")).CountAsync(),
